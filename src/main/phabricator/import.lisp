...
(getf-convenience task id phid title status projects comments owner author ownerphid authorphid description datecreated priority spacephid viewpolicy linked-tasks subscribers actions)
...
(defun convert-task-to-core (task-def)
 (forgerie-core:make-ticket
  ...
  :confidential (or
                 (not (not (find (task-spacephid task-def) *confidential-space-phids* :test #'string=)))
                 (not (find (task-viewpolicy task-def) '("users" "public") :test #'string=)))
